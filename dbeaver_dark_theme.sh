...
main() {
  ...
  if [ "${action}" = "install" ]; then
    echo "Creating backup \"${css_dir_backup}\"..." >&2
    if [ -d "${css_dir_backup}" ]; then
      echo "Creating backup \"${css_dir_backup}\": skipped! Backup already exists!" >&2
    else
      cp -rT "${css_dir_default}" "${css_dir_backup}" || return "$?"
      echo "Creating backup \"${css_dir_backup}\": success!" >&2
    fi

    echo "Applying CSS files from \"${css_files_dir}\"..." >&2
    sudo cp -rT "${css_files_dir}" "${css_dir_default}" || return "$?"
    echo "Applying CSS files from \"${css_files_dir}\": success!" >&2
  elif [ "${action}" = "uninstall" ]; then
    echo "Applying backup \"${css_dir_backup}\"..." >&2
    if [ ! -d "${css_dir_backup}" ]; then
    ...
    echo "Applying backup \"${css_dir_backup}\": success!" >&2
